...
/**
 * Pricing Component - PB_SAMPLE_RETENTION_DISPOSAL:
 *
 * A).  The radio button choices
 *
 * The components using this model could have the following radio button choice question (control) - PS_ITSC_CONTROL:OBJECT_NAME
 * - ‘PRODUCT_TYPE’ - the radio button choices are ‘Chemical’, ‘Petroleum’, or ‘None’
 * - ‘PRODUCT_TYPE1’ - the radio button choices are ‘Chemical’, ‘Petroleum’, or ‘None’
 *
 ...
 *
 * 2.  expressionId
 * - If the question in context is ‘PRODUCT_TYPE’, then:
 * - if the user selected ‘Petroleum’, the expressionId is ‘SRDPetroleum’
 * - If the user selected ‘Chemical’, the expressionId is ‘SRDChemicals’
 *
 * - If the question in context is ‘PRODUCT_TYPE1’, then:
 * - if the user selected ‘Petroleum’, the expressionId is ‘SRDPetroleumBase’
 * - If the user selected ‘Chemical’, the expressionId is ‘SRDChemicalsBase’
 *
 ...
 * Please note here that the only change is the removal of the percentage thing
 *
 * Price:
 *
...
public class PbSampleRetentionDisposalPricer extends StandardPricer
{
  private static Log log = LogFactory.getLog(PbSampleRetentionDisposalPricer.class);
  ...
  public CalculatorResult applyPricingLogic(CalculatorRequest cr)
  {
    ...
    String productType = (String)params.get(Constants.PRODUCT_TYPE);
    String productType1 = (String)params.get(Constants.PRODUCT_TYPE1);
    String expressionId = getExpressionId(productType, productType1);
    ...
    if(sr != null)
    {
      ...
      //params.put(Constants.QUANTITY, new Double(floatQty));

      Calculator calculator = CalculatorFactory.getCalculator(Constants.CALCULATOR_STANDARD_FORMULA);
      if(calculator != null)
      {
        ...
        //calculatorResult.setParameter(Constants.QUANTITY, floatQty);
  ...
  /**
   * Returns the expression id by productionType and productionType1
   *
   * @param productType a String
   * @param productType1 a String
   * @return the expression id as a String
   **/
  public String getExpressionId(String productType, String productType1)
  {
    if(Constants.Petroleum.equals(productType)) return Constants.SRDPetroleum;
    if(Constants.Chemical.equals(productType)) return Constants.SRDChemicals;


    if(Constants.Petroleum.equals(productType1)) return Constants.SRDPetroleumBase;
    if(Constants.Chemical.equals(productType1)) return Constants.SRDChemicalsBase;
