...
public class PbSampleRetentionDisposalPricer extends StandardPricer {
    ...
    public String formatLineDescription(ContractExpression contractExpression, ServiceRate serviceRate, 
        ...
        return this.formatDescription(rbValue, objs, serviceRate.getContractRef());
    }
    ...
    public Object[] getPricingParameters(CfgContract contract, ContractJobOrder contractJobOrder, 
        ...
        String productType = inputInfo.getProductType();
        String productType1 = inputInfo.getProductType1();
        if(Constants.Petroleum.equals(productType)){
            expressionId = Constants.SRDPetroleum;
        }
        else if(Constants.Chemical.equals(productType)){
            expressionId = Constants.SRDChemicals;
        }
        else if(Constants.Petroleum.equals(productType1)){
            expressionId = Constants.SRDPetroleumBase;
        }
        else if(Constants.Chemical.equals(productType1)){
            expressionId = Constants.SRDChemicalsBase;
        }
    ...
    public InputInfo getInputInfo(JobContractService service, JobContractServiceExpression expression, List<JobContractServiceControl> relatedControls) {
        ...
        for (JobContractServiceControl control : relatedControls) {
            ...
            if(Constants.GROUP.equals(control.getControlType())){
                if(Constants.PRODUCT_TYPE.equals(name)){
                    inputInfo.setProductType(value);
                }
                else if(Constants.PRODUCT_TYPE1.equals(name)){
                    inputInfo.setProductType1(value);
                }
            }
