...
<form:form name="attachFilePopUpForm" method="POST" action="job_instruction_attachfile_popup.htm" enctype="multipart/form-data">
	...
	<table width="95%" align="center" border="0" cellpadding="0" cellspacing="0" class="InnerApplTable">
        <tr valign="top">
          <td style="bborder-bottom:#999999 dashed 1px; padding-top:10px;">
			  ...
			  <tr>
           <td><input id="upload" type="submit" value="Upload" name="upload" class="button1" onclick="javascript:top.return_popup_search_result('${command.inputFieldId}',  document.attachFilePopUpForm.file1.value + ';' + document.attachFilePopUpForm.file2.value + ';' + document.attachFilePopUpForm.file3.value + ';' + document.attachFilePopUpForm.file4.value + ';' + document.attachFilePopUpForm.file5.value);top.hideAttachFile('${command.divName}');top.popupboxclose();top.document.attachFilePopUpForm.submit();"/>
	...
	<table width="95%" align="center" border="0" cellpadding="0" cellspacing="0" class="InnerApplTable">
		 <tr>
           <td colspan="2"><b><c:if test="${!empty command.attachments}"><f:message key="attachedFiles"/>:</c:if><c:if test="${empty command.attachments}"><f:message key="noAttachedFiles"/></c:if></b></td>
		 </tr>
			<c:forEach items="${command.attachments}" var="attachs" varStatus="counter">
	        	<tr valign="center">
	        		<td width="40%" align="left">
	        			<a href="view_prebill_attach_popup.htm?systemFileName=${command.attachments[counter.index].systemFileName}" target="_blank">${command.attachments[counter.index].fileName}</a>
		        	</td>
		        	<td align="left">
		        		<form:input cssClass="inputBox" id="descInput${counter}" maxlength="100" size="40" path="attachments[${counter.index}].description" />
						<form:errors path="attachments[${counter.index}].description" cssClass="redstar"/>	        	
			        	
			        	<authz:authorize ifAnyGranted="FileUpload">
			        		<a href="#"><img src="images/icodel.gif" alt="Delete Attachment" width="12" height="14" border="0" onclick="if(confirmAttachDelete()){ onDeleteAttachAttach('${command.attachments[counter.index].id}'); }"></a>
			        	</authz:authorize>
	        ...
	        <c:if test="${command.attachments != null && not empty command.attachments}" >
	        	<tr><td colspan="2">
