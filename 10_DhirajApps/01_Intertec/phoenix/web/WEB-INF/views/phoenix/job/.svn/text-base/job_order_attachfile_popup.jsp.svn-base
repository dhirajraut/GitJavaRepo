...
<form:form name="jobOrderAttachFilePopUpForm" method="POST" action="phx_job_order_attachfile_popup.htm" enctype="multipart/form-data">
    <form:hidden path="divName"/> 
	<form:hidden path="inputFieldId"/>
    <form:hidden path="deleteAttachId" />
    <input type="hidden" name="saveFlag" />
    <form:hidden path="jobOrderNumber"/>
	...
	<table width="95%" align="center" border="0" cellpadding="0" cellspacing="0" class="InnerApplTable">
        <tr valign="top">
          <td style="bborder-bottom:#999999 dashed 1px; padding-top:10px;">
			  ...
			  <tr>
           <td><input id="upload" type="submit" value="Upload" name="upload" class="button1" onclick="javascript:if(doAttachments('${command.inputFieldId}',  document.jobOrderAttachFilePopUpForm.file1.value + ';' + document.jobOrderAttachFilePopUpForm.file2.value + ';' + document.jobOrderAttachFilePopUpForm.file3.value + ';' + document.jobOrderAttachFilePopUpForm.file4.value + ';' + document.jobOrderAttachFilePopUpForm.file5.value)){top.hideAttachFile('${command.divName}');top.popupboxclose();top.document.jobOrderAttachFilePopUpForm.submit();}else{return false;}"/>
           <input id="cancel" type="reset"  value="Cancel" name="cancel"  class="button1"  onclick="javascript:closeAttachPopupWindow('${command.divName}')"/>
		  </td>
		 </tr>
	</table>
	<br>
		 ...
		 <tr>
           <td colspan="2"><b><c:if test="${!empty command.attachmentList}"><f:message key="attachedFiles"/>:</c:if><c:if test="${empty command.attachmentList}"><f:message key="noAttachedFiles"/></c:if></b></td>
			...
			<c:forEach items="${command.attachmentList}" var="attachs" varStatus="counter">
	        	<tr valign="center">
	        		<td width="40%" align="left">
	        			 <a href="view_jobcontract_attach_popup.htm?systemFileName=${command.attachmentList[counter.index].filename}" target="_blank">${command.attachmentList[counter.index].filename}</a>
	              	</td>
	        ...
	        <c:if test="${command.attachmentList != null && not empty command.attachmentList}" >
	        	<tr><td colspan="2">
	        		<input type="button" value="Save" class="button1" onclick="top.hideAttachFile('${command.divName}');top.popupboxclose();onSaveAttach();"/>
	       		</td></tr>
	        </c:if>
  		</table>
	   </form:form>
