package com.quietlyscheming.controls 
{
	...
	import com.quietlyscheming.caching.ContentCache;
...
*-------------------------------------------------------------------------------------------------------------------*/
	...
	[Bindable] public function set smoothing(value:Boolean):void
	{
		_smoothing = value;
...
*-------------------------------------------------------------------------------------------------------------------*/
	...
	private function createOverlaySking():void
	{
		var overlayClass:Class = getStyle("overlaySkin");
		if(overlayClass != null)
		{
			_overlay  = new overlayClass();
		}
	}
	...
	override protected function measure():void
	{
		...
		if(_overlay)
		{
			_overlay.width = _content.width;
			_overlay.height = _content.height;
		}
	}
	...
	override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
	{
		if(_content != null)
		{
			...
			if(smoothing)
			{
				if(_content is Loader && Loader(_content).content)
				{
					Bitmap(Loader(_content).content).smoothing = true;
				}
			}
			if(_maintainAspectRatio)
			{
			...
			if(_border != null)
			{
				...
				if(_overlay)
				{
					_overlay.x =borderMetrics.left;
					_overlay.y = borderMetrics.top;
				}
